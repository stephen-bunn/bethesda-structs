...
from construct import (
    Struct, CString, Enum, FlagsEnum,
    GreedyBytes, GreedyRange, Bytes,
    Float32l, Int64ul, Int32sl, Int32ul, Int16sl, Int16ul, Int8sl, Int8ul,
    ExprAdapter, Optional,
)

from .._common import (SubrecordCollection, Subrecord,)
from ._common import (
    FNVFormID, ServiceFlags,
    ActorValuesEnum, ModEffectEnum,
    SoundLevelEnum, SkillEnum, ImpactMaterialEnum, EquipmentTypeEnum,
    AttackAnimationsEnum, WeaponAnimationEnum, ReloadAnimationEnum,
    RGBAStruct, CTDAStruct, ObjectBoundsStruct,
    ItemCollection, ScriptCollection, DestructionCollection, EffectCollection,
    ModelCollection, Model2Collection, Model3Collection, Model4Collection
)
...
AMMO_Subrecords = SubrecordCollection([
    ...
    Subrecord('RCIL', FNVFormID(['AMEF']) * 'Ammo Effect', optional=True, multiple=True),
])
...
ARMO_Subrecords = SubrecordCollection([
    ...
    ModelCollection,
    Model2Collection,
    Subrecord('ICON', CString('utf8') * 'Male Inventory Icon Filename', optional=True),
    Subrecord('MICO', CString('utf8') * 'Male Message Icon Filename', optional=True),
    Model3Collection,
    Model4Collection,
    Subrecord('ICO2', CString('utf8') * 'Female Inventory Icon Filename', optional=True),
    Subrecord('MIC2', CString('utf8') * 'Female Message Icon Filename', optional=True),
    Subrecord('BMCT', CString('utf8') * 'Ragdoll Constraint Template', optional=True),
    ...
    Subrecord('DNAM', Struct(
        "ar" / ExprAdapter(Int16sl, lambda obj_: (obj_ / 100), lambda obj_: (obj_ * 100)),  # NOTE: value is divided by 100
        "flags" / FlagsEnum(
            Int16ul,
    ...
    Subrecord('BNAM', Enum(
        ...
        no=0,
        yes=1
    ) * 'Overrides Animation Sounds', optional=True),
    ...
    ) * 'Animation Sound', optional=True, multiple=True),
    Subrecord('TNAM', FNVFormID(['ARMO']) * 'Animation Sound Template', optional=True)
])
...
CONT_Subrecords = SubrecordCollection([
    ...
    Subrecord('RNAM', FNVFormID(['SOUN']) * 'Sound - Random / Looping', optional=True),
])
...
DIAL_Subrecords = SubrecordCollection([
    ...
    SubrecordCollection([
        ...
        SubrecordCollection([
            Subrecord('INFC', FNVFormID(['INFO']) * 'Info Connection', optional=True),
            Subrecord('INFX', Int32sl * 'Info Index', optional=True)
        ], optional=True, multiple=True)
    ], optional=True, multiple=True),
    Subrecord('QSTR', FNVFormID(['QUST']) * 'Removed Quest', optional=True, multiple=True),
    SubrecordCollection([
...
FACT_Subrecords = SubrecordCollection([
    ...
    ) * 'Relation', optional=True, multiple=True),
    Subrecord('DATA', Struct(
        "flags_1" / FlagsEnum(
            Int8ul,
            hidden_from_pc=0x01,
...
NAVI_Subrecords = SubrecordCollection([
    ...
    ) * 'Navigation Map Info', optional=True, multiple=True),
    Subrecord('NVCI', Struct(
        "_unknown_0" / FNVFormID(['NAVM']),
        "_unknown_1" / FNVFormID(['NAVM']),
        "_unknown_2" / FNVFormID(['NAVM']),
        "door" / FNVFormID(['REFR']),
    ) * 'Unknown', optional=True, multiple=True)
...
NOTE_Subrecords = SubrecordCollection([
    ...
    Subrecord('SNAM', FNVFormID(['SOUN', 'NPC_', 'CREA']) * 'Sound / Actor', optional=True),
])
...
NPC__Subrecords = SubrecordCollection([
    ...
    Subrecord('SNAM', Struct(
        "faction" / FNVFormID(['FACT']),
        "rank" / Int8ul,
        "unused" / Bytes(3)
    ) * 'Faction', optional=True, multiple=True),
    ...
    Subrecord('SPLO', FNVFormID(['SPEL']) * 'Actor Effect', optional=True, multiple=True),
    Subrecord('EITM', FNVFormID(['ENCH', 'SPEL']) * 'Unarmed Attack Effect', optional=True),
    Subrecord('EAMT', AttackAnimationsEnum * 'Unarmed Attack Animation'),
    DestructionCollection.be(optional=True),
    Subrecord('SCRI', FNVFormID(['SCPT']) * 'Script', optional=True),
    ItemCollection.be(optional=True, multiple=True),
    Subrecord('AIDT', Struct(
        "aggression" / Enum(
            Int8ul,
            unaggressive=0,
            aggressive=1,
    ...
    ) * 'AI Data', optional=True),
    Subrecord('PKID', FNVFormID(['PACK']) * 'Package', optional=True, multiple=True),
    ...
    Subrecord('PNAM', FNVFormID(['HDPT']) * 'Head Part', optional=True, multiple=True),
...
STAT_Subrecords = SubrecordCollection([
    ...
    Subrecord('BRUS', Enum(
        ...
        bush_i=8,
        bush_j=9
    ) * 'Passthrough Sound', optional=True),
    Subrecord('RNAM', FNVFormID(['SOUN']) * 'Sound - Random / Looping', optional=True)
])
...
TES4_Subrecords = SubrecordCollection([
    ...
    Subrecord('ONAM', GreedyRange(Int32ul) * 'Overridden Records', optional=True),  # FIXME: reedy FNV_FormID([REFR, ACHR, ACRE, PMIS, PBEA, PGRE, LAND, NAVM]),
...
WEAP_Subrecords = SubrecordCollection([
    ...
    Subrecord('MWD7', CString('utf8') * 'Model with Mods 1, 2, and 3', optional=True),
    ...
    Subrecord('WNM1', FNVFormID(['STAT']) * 'First Person Model with Mod 1', optional=True),
    Subrecord('WNM2', FNVFormID(['STAT']) * 'First Person Model with Mod 2', optional=True),
    Subrecord('WNM3', FNVFormID(['STAT']) * 'First Person Model with Mods 1 and 2', optional=True),
    Subrecord('WNM4', FNVFormID(['STAT']) * 'First Person Model with Mod 3', optional=True),
    Subrecord('WNM5', FNVFormID(['STAT']) * 'First Person Model with Mods 1 and 3', optional=True),
    Subrecord('WNM6', FNVFormID(['STAT']) * 'First Person Model with Mods 2 and 3', optional=True),
    Subrecord('WNM7', FNVFormID(['STAT']) * 'First Person Model with Mods 1, 2, and 3', optional=True),
    ...
    Subrecord('SNAM', FNVFormID(['SOUN']) * 'Sound - Gun - Shoot 3D', optional=True),
    Subrecord('SNAM', FNVFormID(['SOUN']) * 'Sound - Gun - Shoot Distant', optional=True),
    Subrecord('XNAM', FNVFormID(['SOUN']) * 'Sound - Gun - Shoot 2D', optional=True),
    Subrecord('NAM7', FNVFormID(['SOUN']) * 'Sound - Gun - Shoot 3D Looping', optional=True),
    Subrecord('TNAM', FNVFormID(['SOUN']) * 'Sound - Melee - Swing / Gun - No Ammo', optional=True),
    ...
    Subrecord('WMS1', FNVFormID(['SOUN']) * 'Sound - Mod 1 - Shoot 3D', optional=True),
    Subrecord('WMS1', FNVFormID(['SOUN']) * 'Sound - Mod 1 - Shoot Distant', optional=True),
    Subrecord('WMS2', FNVFormID(['SOUN']) * 'Sound - Mod 1 - Shoot 2D', optional=True),
    Subrecord('DATA', Struct(
        "value" / Int32sl,
        "health" / Int32sl,
        "weight" / Float32l,
